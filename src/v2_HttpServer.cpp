            ...
            else if ( !requestCtxPtr->hasHeader )
            {
                ...
                if ( requestCtxPtr->requestContentLength > 0 )
                {
                    ...
                    if ( data.getSize() == requestCtxPtr->requestContentLength ) // 收到的数据等于请求体数据的大小
                    {
                        ...
                        this->_pool.task( &HttpServer::onClientRequestInternal, this, requestCtxPtr, std::ref(requestCtxPtr->header), std::ref(requestCtxPtr->body) ).post();
                        // 清空数据
                        data.free();
...
{
    return new HttpRequestCtx( _app, clientId, clientEpStr, clientSockPtr );
}
...
// 收到一个请求
void HttpServer::onClientRequestInternal( winux::SharedPointer<HttpRequestCtx> requestCtxPtr, http::Header & header, winux::AnsiString & body )
{
    if ( this->_verbose )
    {

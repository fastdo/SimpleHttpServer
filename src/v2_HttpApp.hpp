...
public:
    ...
    void crossRoute( winux::String const & method, winux::String const & path, HttpServer::CrossRouteHandlerFunction handler ) { _server.crossRoute( method, path, handler ); }

    /** \brief 注册普通路由处理器 method可以是*表示通配所有HTTP方法，path需以/开头 */
    void route( winux::String const & method, winux::String const & path, HttpServer::RouteHandlerFunction handler ) { _server.route( method, path, handler ); }
